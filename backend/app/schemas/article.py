...
class ArticleCreate(BaseModel):
    ...
    def validate_image_url(cls, v: str | None) -> str | None:
        ...
        if not re.match(r"^https?://.+\.(jpg|jpeg|png|webp|gif)(\?.*)?$", v, re.I):
            raise ValueError("image_url doit pointer vers une image valide")
        return v
    @model_validator(mode="after")
    def description_not_same_as_name(self) -> Self:
        if self.description.lower().strip() == self.name.lower().strip():
            raise ValueError("La description ne peut pas être identique au nom")
        return self
